...
"""
ANSIBLE: Dynamic Inventory Generator for CVIM-MON HA Installer.
"""
...
INVENTORY_FILE = "cvimmon_inventory.yaml"
...
def list_groups():
    ...
    inventory_file = os.path.join(cfg_dir, INVENTORY_FILE)

    required_files = [setup_file, defaults_file, inventory_file]
...
def parse_hosts_file():
    ...
    hosts_file = os.path.join(cfg_dir, INVENTORY_FILE)
...
class Inventory:
    ...
    def generate_all_hosts(self):
        ...
        for host, attributes in self.entries.iteritems():
            self.processed_entries['all'].append(
                attributes['ansible_ssh_host'])
...
def main():
    ...
    inventory_args = {'entries': hosts_entries,
